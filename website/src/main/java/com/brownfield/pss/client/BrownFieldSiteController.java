...
public class BrownFieldSiteController {
   ...
   public String searchBookingForm(Model model) {
	...
	public String bookQuery(@PathVariable String flightNumber, 
			...
			//create baggage
			long luggageId = createDefaultBaggage(checkinId);
	   		model.addAttribute("luggageMessage","The Luggage is successful added with ID: "+ luggageId);
	   		model.addAttribute("checkinId","Please check it in H2 DB or go to link localhost:9090/luggage/getLuggage?checkInId=" + checkinId);
	   		return "checkinconfirm"; 
	}	
	
	private long createDefaultBaggage(long checkinId) {
		try {
			String urlGateway = "http://luggage-apigateway/api/luggage/createLuggage";
			Luggage luggage = new Luggage(checkinId,1000,30);
			long baggageId = luggageClient.postForObject(urlGateway,luggage,long.class);
			return baggageId;
		}
		catch(Exception ex) {
			return 0;
